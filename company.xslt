...
<!--Designed and generated by Altova StyleVision Enterprise Edition 2015 sp1 - see http://www.altova.com/stylevision for more information.-->
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:altova="http://www.altova.com" xmlns:altovaext="http://www.altova.com/xslt-extensions" xmlns:clitype="clitype" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:iso4217="http://www.xbrl.org/2003/iso4217" xmlns:ix="http://www.xbrl.org/2008/inlineXBRL" xmlns:java="java" xmlns:link="http://www.xbrl.org/2003/linkbase" xmlns:map="http://www.w3.org/2005/xpath-functions/map" xmlns:math="http://www.w3.org/2005/xpath-functions/math" xmlns:sps="http://www.altova.com/StyleVision/user-xpath-functions" xmlns:vc="http://www.w3.org/2007/XMLSchema-versioning" xmlns:xbrldi="http://xbrl.org/2006/xbrldi" xmlns:xbrli="http://www.xbrl.org/2003/instance" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" exclude-result-prefixes="#all">
	...
	<xsl:param name="SV_GeneratedFileNamePrefix" select="if ( $SV_BaseOutputFileName ) then $SV_BaseOutputFileName else &apos;company&apos;" as="xs:string?"/>
	<xsl:variable name="XML" select="/"/>
	<xsl:variable name="altova:nPxPerIn" select="96"/>
	<xsl:import-schema schema-location="company.xsd" use-when="system-property('xsl:is-schema-aware')='yes'"/>
	...
	<xsl:template name="altova:Root">
		<html>
			<head>
				<title/>
				<meta name="generator" content="Altova StyleVision Enterprise Edition 2015 sp1 (http://www.altova.com)"/>
			...
			<body>
				...
				<xsl:for-each select="$XML">
					<xsl:for-each select="Company">
						<xsl:for-each select="Employee">
							<xsl:variable name="EmpIdVar" select="@empId"/>
							<xsl:variable name="MaxPer" select="max(WorksFor[@empId =$EmpIdVar]/percentTime)"/>
							<xsl:variable name="MaxDiv" select="WorksFor[percentTime=$MaxPer]/@divId"/>
							<xsl:variable name="LastDiv" select="//WorksFor[@empId = $EmpIdVar]/@divId[fn:last()]"/>
							<xsl:variable name="LastDept" select="//Department/Division[@divId = //WorksFor[@divId = //@divId][fn:last()]/@divId]/../@deptId"/>
							<xsl:variable name="LastMDiv" select="(//Division[@managerEmpId = $EmpIdVar])[fn:last()]/@divId"/>
							<xsl:variable name="Mcount" select="count(//Division[@managerEmpId=$EmpIdVar])"/>
							<br/>
							<span>
								<xsl:text>Employee </xsl:text>
							</span>
							...
							<span style="font-weight:bold; ">
								<xsl:value-of select="count( WorksFor )"/>
							</span>
							<span>
								<xsl:text> division(s),&#160; which are </xsl:text>
							</span>
							<xsl:for-each select="WorksFor">
								<xsl:variable name="DivIdVar" select="@divId"/>
								<xsl:for-each select="$XML">
									<xsl:for-each select="Company">
										<xsl:for-each select="Department">
											<xsl:for-each select="Division[@divId = $DivIdVar and @divId != $LastDiv[fn:last()]]">
												<xsl:for-each select="divName">
													<span style="font-weight:bold; ">
														<xsl:apply-templates/>
													</span>
													<span style="font-weight:bold; ">
							...
							<span style="font-weight:bold; ">
								<xsl:value-of select="//Division[@divId = $LastDiv[fn:last()]]/divName"/>
							</span>
							<span>
								<xsl:text>. </xsl:text>
							</span>
							...
							<span style="font-weight:bold; ">
								<xsl:value-of select="count(//Division[@managerEmpId=$EmpIdVar])"/>
							</span>
							<span>
								<xsl:text> division(s)</xsl:text>
							</span>
							<span>
								<xsl:value-of select="if($Mcount&gt;0) then &apos;, which are  &apos; else &apos;&apos;"/>
							</span>
							<xsl:for-each select="$XML">
								<xsl:for-each select="Company">
									<xsl:for-each select="Department">
										<xsl:for-each select="Division[@managerEmpId = $EmpIdVar and @divId != $LastMDiv]">
											<xsl:for-each select="divName[$Mcount &gt;1]">
												<span style="font-weight:bold; ">
													<xsl:apply-templates/>
												</span>
												<span>
							...
							<span>
								<xsl:value-of select="if($Mcount&gt;1) then &apos;and&apos; else &apos;&apos;"/>
							</span>
							<span>
								<xsl:value-of select="if($Mcount&gt;1)then &apos; &apos; else &apos;&apos;"/>
							</span>
							<xsl:for-each select="$XML">
								<span style="font-weight:bold; ">
									<xsl:value-of select="//Division[@divId = $LastMDiv]/divName"/>
								</span>
								<xsl:for-each select="Company">
									<xsl:for-each select="Department[fn:last()]"/>
								</xsl:for-each>
							</xsl:for-each>
							<span>
								<xsl:text>. </xsl:text>
							...
							<xsl:for-each select="$XML">
								<xsl:for-each select="Company">
									<xsl:for-each select="Department">
										<xsl:for-each select="Division[@divId = $MaxDiv]">
											<xsl:for-each select="divName">
												<span style="font-weight:bold; ">
													<xsl:apply-templates/>
												</span>
				...
				<br/>
				<h1 style="background-color:#8be880; ">
					<span style="color:red; ">
						<xsl:text>Department information</xsl:text>
					</span>
				</h1>
				<xsl:for-each select="$XML">
					<xsl:for-each select="Company">
						<xsl:for-each select="Department">
							...
							<span style="font-weight:bold; ">
								<xsl:value-of select="count(Division)"/>
							</span>
							<span>
								<xsl:text> division(s): </xsl:text>
							</span>
							<xsl:for-each select="Division[fn:position() &lt; fn:last()]">
								<xsl:for-each select="divName">
									<span style="font-weight:bold; ">
										<xsl:apply-templates/>
									</span>
							...
							<xsl:for-each select="Division[fn:last()]">
								<xsl:for-each select="divName">
									<span style="font-weight:bold; ">
										<xsl:apply-templates/>
									</span>
